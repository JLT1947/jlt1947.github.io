...
//
	var r, loop, genomap, inode, hammer,loaded = true, map, cSvg, hSvg, wSvg, ocSvg, ohSvg, owSvg, s, panZoom;
	var root = null;
...
function svgInit(info)
{
	...
	if (param.max) togglePopUpFrame(null, '../');
	if (param.id) {
		inode = genomap.querySelector('g[id='+param.id+']');
		if (inode) {
			map.X = parseInt(inode.getAttribute('data-x')) + dX;
			map.Y = parseInt(inode.getAttribute('data-y')) + dY;
			map.Highlight = true;
		}
	}
